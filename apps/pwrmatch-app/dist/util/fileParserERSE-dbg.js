sap.ui.define([
	...
	function _loadComConditionsFile(sPath, that) {
		...
		fetch(sPath) // adapta "meuapp" ao namespace do projeto
			...
			.then(arrayBuffer => {
				var oModel = that.getOwnModels().appFlowModel;
				var comConditions = oModel.oData.comConditionsERSE;
				const e = { target: { result: arrayBuffer } };
				var data = new Uint8Array(e.target.result);
				var workbook = XLSX.read(data, { type: "array" });
				// Reads first sheet to model
	...
	function _loadComConditionsPricesFile(sPath, that) {
		...
		fetch(sPath) // adapta "meuapp" ao namespace do projeto
			...
			.then(arrayBuffer => {
				var oModel = that.getOwnModels().appFlowModel;
				var comConditionsPrices = oModel.oData.comConditionsPricesERSE;
				const e = { target: { result: arrayBuffer } };
				var data = new Uint8Array(e.target.result);
				var workbook = XLSX.read(data, { type: "array" });
				// Reads first sheet to model
	...
	function _loadComConditionsPricesMetadataFile(sPath, that) {
		//fetch(sap.ui.require.toUrl(sPath)) // adapta "meuapp" ao namespace do projeto
		fetch(sPath) // adapta "meuapp" ao namespace do projeto
			.then(response => {
				if (!response.ok) {
			...
			.then(arrayBuffer => {
				var oModel = that.getOwnModels().appFlowModel;
				var comConditionsPricesMetadata = oModel.oData.comConditionsPricesMetadataERSE;
				const e = { target: { result: arrayBuffer } };
				var data = new Uint8Array(e.target.result);
				var workbook = XLSX.read(data, { type: "array" });
				// Reads first sheet to model
				...
				var condComerciais = jsonData
					.slice(17, 83) // ignora cabeçalho
					.map(item => ({
						id: item[0],
						descr: item[1]
					}));

				comConditionsPricesMetadata.push({
	...
	// Load regulated market tariffs and create related model
	function _loadRegMrktTariffsFile(sPath, that) {
		fetch(sPath) // adapta "meuapp" ao namespace do projeto
			.then(response => {
				if (!response.ok) {
					throw new Error("Erro ao carregar ficheiro: " + sPath);
			...
			.then(arrayBuffer => {
				var oModel = that.getOwnModels().appFlowModel;
				var regMrktTariffs = oModel.oData.regMrktTariffsERSE;
				const e = { target: { result: arrayBuffer } };
				var data = new Uint8Array(e.target.result);
				var workbook = XLSX.read(data, { type: "array" });
				// Reads first sheet to model
				...
				jsonData.forEach(function (item) {
					if (item.COM && item.COD_Proposta) {
						regMrktTariffs.push(item);
						//var exists = regMrktTariffs.some(function (s) {
						//	return s.COM === item.COM && s.COD_Proposta === item.COD_Proposta;
						//});
						//if (!exists) {
						//	regMrktTariffs.push(item);
	...
	return {
		parseERSEFiles: function (that) {
			...
			try {
				...
				_loadComConditionsPricesMetadataFile("data/erse/metadata.xlsx", that);
				// regulated market tariffs
				_loadRegMrktTariffsFile("data/erse/lstTarifas_RegM.xlsx", that);
